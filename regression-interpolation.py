...
def create_interpolated_dataframes(propeller_dict, rpm_min=None, rpm_max=None, num_points=100):
    ...
    for i, (df_name, df) in enumerate(propeller_dict.items()):
        ...
        try:
            ...
            # Sort by RPM and remove duplicate RPM entries (interp/spline require strictly increasing x)
            sort_idx = np.argsort(rpm_original)
            rpm_sorted = rpm_original[sort_idx]
            ct_sorted = ct_original[sort_idx]
            cp_sorted = cp_original[sort_idx]

            # Remove duplicate RPMs by keeping the first occurrence
            rpm_unique, unique_idx = np.unique(rpm_sorted, return_index=True)
            ct_unique = ct_sorted[unique_idx]
            ...
            # Choose cubic spline (UnivariateSpline with k=3) when enough points, otherwise fallback to linear interp1d
            try:
                if len(rpm_unique) >= 4:
                    # Exact cubic spline through points (s=0). Use k=3 for cubic.
                    ct_spline = UnivariateSpline(rpm_unique, ct_unique, k=3, s=0)
                    cp_spline = UnivariateSpline(rpm_unique, cp_unique, k=3, s=0)
                    ct_interp = ct_spline(rpm_interp)
                    cp_interp = cp_spline(rpm_interp)
                elif len(rpm_unique) >= 2:
                    # Not enough points for cubic spline: fallback to linear interpolation
                    ct_interp_func = interp1d(rpm_unique, ct_unique, kind='linear', bounds_error=False, fill_value='extrapolate')
                    cp_interp_func = interp1d(rpm_unique, cp_unique, kind='linear', bounds_error=False, fill_value='extrapolate')
                    ct_interp = ct_interp_func(rpm_interp)
                    cp_interp = cp_interp_func(rpm_interp)
                else:
                    raise ValueError("Not enough unique RPM points to interpolate")
            except Exception as e:
                print(f"Interpolation error for {df_name}: {e}")
                continue
